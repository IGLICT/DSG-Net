...
class PartDeformEncoder2(nn.Module):
    ...
    def forward(self, featurein):
        ...
        # print(net.shape)
        net_logr = torch.tanh(self.mlp1_logr(net_logr.reshape(net_logr.size(0), self.vertex_num * 3)))
        net_s = torch.tanh(self.mlp1_s(net_s.reshape(net_logr.size(0), self.vertex_num * 6)))
